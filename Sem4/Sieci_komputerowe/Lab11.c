...
void handle_client(int client_fd) {
    ...
    // Odbierz zapytanie (ignorujemy jego treść)
    recv(client_fd, buffer, BUFSIZE - 1, 0);

    // Odczytaj uptime z /proc/uptime
    ...
    // Wyślij nagłówki i ciało
    send(client_fd, header, header_len, 0);
    send(client_fd, body, body_len, 0);

    // Zamknij strumień wyjściowy i połączenie
...
int main(int argc, char* argv[]) {
    ...
    // Zablokuj uruchomienie jako root
    if (getuid() == 0) {
    ...
    addr.sin_addr.s_addr = htonl(INADDR_ANY);  // Zmienna na INADDR_LOOPBACK -> serwer lokalny
